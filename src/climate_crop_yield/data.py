...
def _country_code_mask(codes: pd.Series) -> pd.Series:
    """Keep three-letter country/territory codes and reject OWID aggregate codes."""
...
def load_crop_yields(crops: Iterable[str] | None = None) -> pd.DataFrame:
    ...
    for crop in selected:
        ...
        part = read_owid_series(CROP_SLUGS[crop], value_name="yield_t_ha")
        part["crop"] = crop
        frames.append(part)
...
def load_country_year_drivers() -> pd.DataFrame:
    ...
    for frame in frames[1:]:
        # Code + Year are the stable join keys. Entity is kept from the first frame.
        value_cols = [c for c in frame.columns if c not in {"Entity", "Code", "Year"}]
        panel = panel.merge(
            frame[["Code", "Year"] + value_cols],
            on=["Code", "Year"],
            how="outer",
            validate="one_to_one",
        )
...
def build_analysis_panel(
    ...
    # Yield-side entity names are canonical for the final crop panel.
    panel = yields.merge(
        drivers.drop(columns=["Entity"]),
        on=["Code", "Year"],
        how="left",
    ...
    if countries is not None:
        panel = panel[panel["Code"].isin(set(countries))].copy()
